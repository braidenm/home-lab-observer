...
func (s credentialSource) Credential(ctx context.Context, b uploadstate.Binding) (string, error) {
	if ctx.Err() != nil || s.record.ServerID != b.ServerID || s.record.ConnectorID != b.ConnectorID {
...
func run(ctx context.Context) (code int) {
	...
	if _, err := connectedidentity.Resolve(releaseIdentity, "uploader"); err != nil {
		return 22
	}
	if len(os.Args) == 2 {
		return connectedenroll.Run(ctx, os.Args[1], os.Stdin, os.Stdout)
	}
	if len(os.Args) != 1 {
		return 22
	}
