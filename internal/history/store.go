...
func migrate(ctx context.Context, db *sql.DB) error {
	...
	defer tx.Rollback()
	statements := []string{
		`CREATE TABLE IF NOT EXISTS samples(metric TEXT NOT NULL CHECK(metric IN ('cpu.utilization_percent','memory.utilization_percent','filesystem.aggregate_utilization_percent','network.receive_bytes_per_second','network.transmit_bytes_per_second','process.count')), observed_at_ns INTEGER NOT NULL, value REAL NOT NULL, PRIMARY KEY(metric, observed_at_ns)) WITHOUT ROWID`,
		`CREATE TABLE IF NOT EXISTS rollups(metric TEXT NOT NULL CHECK(metric IN ('cpu.utilization_percent','memory.utilization_percent','filesystem.aggregate_utilization_percent','network.receive_bytes_per_second','network.transmit_bytes_per_second','process.count')), bucket_start_ns INTEGER NOT NULL, resolution_seconds INTEGER NOT NULL, sample_count INTEGER NOT NULL, minimum REAL NOT NULL, maximum REAL NOT NULL, total REAL NOT NULL, last_value REAL NOT NULL, last_at_ns INTEGER NOT NULL, PRIMARY KEY(metric, bucket_start_ns, resolution_seconds)) WITHOUT ROWID`,
