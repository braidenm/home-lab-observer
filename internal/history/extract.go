...
func Extract(snapshot observation.Snapshot, previous *observation.Snapshot) []Sample {
	...
	if usable(snapshot.CPU.State) && snapshot.CPU.Data != nil {
		result = append(result, Sample{Metric: CPUUtilization, At: at, Value: snapshot.CPU.Data.UsagePercent})
	}
	if usable(snapshot.Memory.State) && snapshot.Memory.Data != nil {
		result = append(result, Sample{Metric: MemoryUtilization, At: at, Value: snapshot.Memory.Data.UsagePercent})
	}
	if usable(snapshot.Filesystems.State) && snapshot.Filesystems.Data != nil {
		...
		if total > 0 {
			result = append(result, Sample{Metric: FilesystemUtilization, At: at, Value: float64(used) * 100 / float64(total)})
		}
	}
	if usable(snapshot.Processes.State) && snapshot.Processes.Data != nil {
		...
		result = append(result, Sample{Metric: ProcessCount, At: at, Value: float64(count)})
	}
	if previous != nil && usable(snapshot.Network.State) && usable(previous.Network.State) && snapshot.Network.Data != nil && previous.Network.Data != nil {
		...
		if seconds > 0 && snapshot.Network.Data.BytesRecv >= previous.Network.Data.BytesRecv && snapshot.Network.Data.BytesSent >= previous.Network.Data.BytesSent {
			result = append(result, Sample{Metric: NetworkReceiveRate, At: at, Value: float64(snapshot.Network.Data.BytesRecv-previous.Network.Data.BytesRecv) / seconds})
			result = append(result, Sample{Metric: NetworkTransmitRate, At: at, Value: float64(snapshot.Network.Data.BytesSent-previous.Network.Data.BytesSent) / seconds})
		}
	}
	return result
}
