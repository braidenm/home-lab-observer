...
func auditPrincipals(parent context.Context, c connectedprofile.Config) error {
	...
	for _, path := range []string{"/run/systemd/userdb", "/etc/userdb", "/run/userdb", "/run/host/userdb", "/usr/lib/userdb"} {
	...
	for _, user := range []struct {
		...
		out, err := command(ctx, "/usr/bin/getent", []string{"initgroups", user.name}, nil, 4096)
		if err != nil || !exactInitgroups(out, user.name, user.groups) {
			return ErrUnsafe
		}
		out, err = command(ctx, "/usr/bin/passwd", []string{"--status", "--", user.name}, nil, 1024)
	...
	for _, group := range []struct {
		...
		for _, key := range []string{group.name, strconv.FormatUint(uint64(group.gid), 10)} {
			out, err := command(ctx, "/usr/bin/getent", []string{"group", key}, nil, 4096)
			if err != nil || string(out) != snapshot.groups[group.name]+"\n" {
				return ErrUnsafe
			}
		}
		// Query only our two dedicated groups, never the host's full shadow data.
		out, err := command(ctx, "/usr/bin/getent", []string{"gshadow", group.name}, nil, 4096)
