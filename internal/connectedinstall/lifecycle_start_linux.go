...
func startInstalled(ctx context.Context) error {
	return startInstalledWith(ctx, startOperations{
		...
		preflight: preflight, inspect: inspectInstalled, removal: rejectRemoval,
...
func rejectRemoval() error {
	parent, err := rootDirectory(connectedprofile.ConfigDirectory)
	if err != nil {
		return ErrUnsafe
	}
	defer parent.Close()
	for _, name := range []string{"uninstalling.json", "uninstalled.json"} {
		// A marker of any type, including a dangling symlink, refuses activation.
