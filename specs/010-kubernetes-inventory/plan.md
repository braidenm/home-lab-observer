...
2. Freeze a versioned normalized domain model plus closed JSON Schema/OpenAPI contracts for capabilities and current
   inventory. Include nullable totals, per-stream quality, per-Pod/global container-child counts and explicit
   local-sensitive/no-upload policy.
...
4. Implement the fair bounded collector and safe projection. Decode one bounded object at a time, discard the raw value,
   derive installation-local UID aliases and retain only the selected Pod, per-container-status and controller fields.
   Apply the per-Pod 16-child and global 800-child budgets with deterministic round-robin selection, then enforce the
   final 1 MiB encoded response cap with explicit omitted/truncated counts. Add canaries for every prohibited field.
...
Pagination consistency is scoped to one `(namespace,kind)` stream, not the whole cluster response. A `410` restart clears
all staged records, counts and continuation state for that stream before page one is retried. Totals are published only
after one unchanged-resourceVersion sequence reaches an empty continuation; `remainingItemCount`, page size and other
streams never supply an inferred total. Within retained Pods, container children are sorted and selected fairly as
specified so a large Pod cannot hide all other container status while staying within the global and response byte caps.
...
- Domain/contract: exact fields and enums, aliases, null semantics, per-stream count invariants, response cap, unknown
  additive client fields, malformed producer output, per-Pod/global container child counts and deterministic omissions.
...
- Projection/privacy: full-object fixtures containing tokens, environment, labels, annotations, images/image IDs,
  container IDs, volumes, node/IP data and condition/container-state text; none survives normalization, caching, API
  encoding, logs or metric labels. Zero, one, multiple and malformed controller owner references prove null/discard
  behavior without label/name inference.
- UI: legacy source without the optional method, independently aborted/raced loads, namespace/kind filters, honest
  partial/empty states, bounded Pods/Containers/Controllers views, keyboard access and responsive contained data at 390,
...
events, logs, watches, history/upload, alerts, actions, Helm and Kubernetes dashboard embedding are separate work. Spec
011 covers Docker actions only; it does not authorize Kubernetes actions or reuse this observation credential.
