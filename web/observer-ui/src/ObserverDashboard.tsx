...
export function ObserverDashboard({ dataSource, initialView = "overview", mode = "local", onViewChange }: ObserverDashboardProps) {
  ...
  const [logRange, setLogRange] = useState<TrendRange>("6h");
  const { capabilities, snapshot, trends, containerInventory, diagnosticsHealth, logSummary, refreshedAt, isLoading, refresh } = useObserverData(dataSource, range, logRange);
  ...
  return (
    <div className="observer-shell">
      ...
      <main id={contentId} tabIndex={-1}>
        ...
        {activeView === "logs" && <LogsView snapshot={snapshot} capabilities={capabilities.value} summary={logSummary} range={logRange} onRangeChange={setLogRange} />}
        {activeView === "health" && <HealthPrivacyView capabilities={capabilities.value} snapshot={snapshot.value} diagnosticsHealth={diagnosticsHealth} mode={mode} hasContainerInventory={dataSource.getContainerInventory !== undefined} />}
      </main>
